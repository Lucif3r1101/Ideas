...
export async function sendConfirmation(
  ...
  const senderName = process.env.BREVO_SENDER_NAME || "Thumb";
  ...
  try {
    const res = await fetch(ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        to: [{ email: to }],
        subject: "You're on the Thumb list",
...
function text(answer: string) {
  return [
    "You're on the list.",
    "",
    "Thanks for signing up to Thumb. We'll email you once, when access opens. Nothing in between.",
...
<html><body style="margin:0;background:#EAF3FF;padding:28px 16px">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:520px;margin:0 auto;background:#fff;border-radius:18px;overflow:hidden">
    <tr><td style="background:#6C4CE0;padding:26px 28px">
      <div style="font:700 24px/1 system-ui,sans-serif;letter-spacing:-.02em;color:#fff">Thumb</div>
    </td></tr>
